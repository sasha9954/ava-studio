...
function buildReadiness({ modeId, audio, normalizedScenes, productionScenes, assets }) {
  ...
  const imageAwareVideoPromptsReady = stillsReady && stillScenes.length > 0 && imageAwareScenes.length === stillScenes.length
  ...
  return {
    ...
    next_action: hasGeneratedStills && !imageAwareVideoPromptsReady ? 'Run image-aware video prompt pass' : '',
