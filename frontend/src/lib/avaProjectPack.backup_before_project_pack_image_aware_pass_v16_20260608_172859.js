...
function productionScenesFromNormalized(normalizedScenes = [], modeId = 'manual_general_v1') {
  return normalizedScenes.map((scene, index) => {
    ...
    return {
      ...
      video_motion_prompt: prompts.videoMotion,
      video_motion_negative: prompts.videoMotionNegative,
      lipsync_motion_prompt: prompts.lipsync,
      positive_prompt: prompts.positive,
      negative_prompt: prompts.negative,
      video_prompt: firstText(scene.video_prompt, prompts.positive),
      prompt_positive: firstText(scene.prompt_positive, prompts.positive),
      prompt_negative: firstText(scene.prompt_negative, prompts.negative),
...
function buildReadiness({ modeId, audio, normalizedScenes, productionScenes, assets }) {
  ...
  const fullPackage = hasAudio && hasAudioDuration && hasScenes && hasProductionPrompts && hasGeneratedStills
  const stage = fullPackage ? 'images_ready' : promptsReady ? 'prompts_ready' : sceneSplitReady ? 'scene_split_ready' : hasScenes ? 'scene_split_import_ready' : 'created'
