// AVA_PROJECT_PACK_SCENE_IMPORT_EXPORT_V14: root scenes[] and manual-timing import aliases in unified pack.
// AVA_PROJECT_PACK_MMAUDIO_SOUND_V10: optional scene sound / MMAudio fields in project pack.
...
function contextValue(key, ...sources) {
  for (const source of sources) {
    const obj = asObject(source)
    const value = firstText(obj[key], obj.mode?.[key], obj.project_mode?.[key], obj.metadata?.[key])
...
function audioAssetFromManualTiming(manualTiming = {}, board = {}, summary = {}) {
  const audio = asObject(manualTiming.audio || manualTiming.audio_file || manualTiming.sourceAudio)
  const assetId = firstText(audio.asset_id, audio.assetId, manualTiming.audio_asset_id, manualTiming.audioAssetId, manualTiming.asset_id, manualTiming.assetId, board.audio_asset_id, board.audioAssetId)
  const apiPath = firstText(audio.asset_api_path, audio.assetApiPath, audio.api_path, audio.apiPath, manualTiming.audio_api_path, manualTiming.audioApiPath, board.audio_api_path, board.audioApiPath)
...
function hasStill(scene = {}) {
  return Boolean(firstText(scene.image_url, scene.imageUrl, scene.photo_url, scene.photoUrl, scene.start_image_url, scene.startImageUrl, scene.generated_still_url, scene.generatedStillUrl, scene.video_asset_id && scene.video_url))
}
...
function productionScenesFromBoard(board = {}, timingScenes = [], modeId = 'manual_general_v1') {
  ...
  return fallbackScenes.map((scene, index) => {
    ...
    const base = {
      ...
      photo_prompt_positive: firstText(scene.photo_prompt_positive, scene.image_prompt, scene.imagePrompt),
      photo_prompt_negative: firstText(scene.photo_prompt_negative),
      video_motion_prompt: firstText(scene.video_motion_prompt, scene.motion_prompt, scene.motionPrompt),
      video_motion_negative: firstText(scene.video_motion_negative, scene.motion_negative),
...
function buildReadiness({ project, manualTiming, board, summary, modeId, format, audio, timingScenes, productionScenes, assets }) {
  const missing = []
  const warnings = []
  ...
  if (soundMissingScenes.length) {
    ...
    next_actions_for_user.push('Для сцен с включённым sound_design_needed заполни mmaudio_prompt / scene_ambience_prompt / foley_prompt.')
  }
  ...
  if (modeId === 'recipe_process_v1') {
    if (!contextValue('recipe_topic', project, manualTiming, board, summary)) missing.push('mode.recipe_topic')
  ...
  if (modeId === 'lyric_meaning_remix_v1') {
    if (!contextValue('story_world', project, manualTiming, board, summary)) missing.push('mode.story_world')
    if (!contextValue('twist_object', project, manualTiming, board, summary)) missing.push('mode.twist_object')
    ...
    next_questions.push({ id: 'reveal_logic', required: true, question_ru: 'Как должен читаться reveal: сразу, в середине или в финале?' })
  }

  if (modeId === 'video_first_documentary_v1') {
    if (!assets.source_video?.path) {
    ...
    if (!contextValue('source_shot_index', project, manualTiming, board, summary)) missing.push('video_first.source_shot_index')
    if (!contextValue('clean_cut_verification', project, manualTiming, board, summary)) missing.push('video_first.clean_cut_verification')
  }

  if (modeId === 'music_visual_story_v1' && !contextValue('visual_style', project, manualTiming, board, summary)) {
    next_questions.push({ id: 'visual_style', required: false, question_ru: 'Какой визуальный стиль клипа держим на протяжении всего видео?' })
  }
...
function expectedOutputsForMode(modeId = 'manual_general_v1') {
  if (modeId === 'recipe_process_v1') return ['storyboard_locked.md', 'storyboard_locked.json', 'prompts_pack.json', 'board_import_ready.json', 'generated_stills_manifest.json', 'contact_sheet_labeled.jpg', 'validation_report.json']
  if (modeId === 'lyric_meaning_remix_v1') return ['storyboard_locked.md', 'prompts_pack.json', 'board_import_ready.json', 'generated_stills_manifest.json', 'contact_sheet_labeled.jpg', 'validation_report.json']
  if (modeId === 'video_first_documentary_v1') return ['source_shots.json', 'contact_sheets', 'video_match_board_v2.json', 'validation_report.json']
  return ['storyboard_locked.json', 'prompts_pack.json', 'board_import_ready.json', 'validation_report.json']
}


function buildManualTimingImportScenesV14({ timingScenes = [], productionScenes = [], modeId = 'manual_general_v1' } = {}) {
  const productionById = new Map(
    (Array.isArray(productionScenes) ? productionScenes : [])
      .map((scene, index) => [String(scene.scene_id || scene.id || `seg_${String(index + 1).padStart(2, '0')}`), scene])
  )

  const source = Array.isArray(timingScenes) && timingScenes.length
    ? timingScenes
    : (Array.isArray(productionScenes) ? productionScenes : [])

  return source.map((scene, index) => {
    const id = firstText(scene.scene_id, scene.id) || `seg_${String(index + 1).padStart(2, '0')}`
    const production = productionById.get(String(id)) || {}
    const start = Number(scene.start ?? scene.start_sec ?? scene.target_t0 ?? production.start ?? production.start_sec ?? production.target_t0 ?? 0) || 0
    const endRaw = Number(scene.end ?? scene.end_sec ?? scene.target_t1 ?? production.end ?? production.end_sec ?? production.target_t1 ?? 0) || 0
    const durationRaw = Number(scene.duration ?? scene.duration_sec ?? production.duration ?? production.duration_sec ?? 0) || 0
    const end = endRaw > start ? endRaw : start + Math.max(0, durationRaw)
    const duration = Math.max(0, Number((end - start).toFixed(3)))
    const route = firstText(scene.route, scene.planned_route, production.route, production.planned_route) || 'i2v'
    const common = {
      id,
      scene_id: id,
      start,
      ...
      target_t1: end,
      route,
      planned_route: firstText(scene.planned_route, production.planned_route, route),
      scene_word_text: firstText(scene.scene_word_text, scene.text, production.scene_word_text, production.text),
      ...
      translated_text_ru: firstText(scene.translated_text_ru, scene.translation, scene.translation_ru, production.translated_text_ru, production.translation),
      meaning_hint_ru: firstText(scene.meaning_hint_ru, scene.meaning, production.meaning_hint_ru, production.meaning),
      blockId: firstText(scene.blockId, scene.block_id, production.blockId, production.block_id),
      blockTitle: firstText(scene.blockTitle, scene.block_title, production.blockTitle, production.block_title),
      color: firstText(scene.color, scene.scene_color, production.color, production.scene_color),
      visual_action: firstText(production.visual_action, scene.visual_action),
      viewer_should_understand: firstText(production.viewer_should_understand, scene.viewer_should_understand, production.meaning, scene.meaning),
      readability_check: firstText(production.readability_check, scene.readability_check),
      locked: true,
      do_not_change_scene_id: true,
      do_not_change_start_end_duration: true,
    }
    ...
    if (modeId === 'recipe_process_v1') {
      common.recipe_step = firstText(production.recipe_step, scene.recipe_step, production.blockTitle, scene.blockTitle)
    }

    if (modeId === 'lyric_meaning_remix_v1') {
      common.idea_fn = firstText(production.idea_fn, scene.idea_fn)
      common.idea_meaning = firstText(production.idea_meaning, scene.idea_meaning, common.meaning_hint_ru)
      common.idea_story = firstText(production.idea_story, scene.idea_story)
      common.idea_anchor = firstText(production.idea_anchor, scene.idea_anchor)
    }
...
export function buildAvaProjectPackV1({ project = {}, manualTiming = {}, board = {}, summary = {}, includeLegacyRaw = false } = {}) {
  ...
  const rootScenesForManualTimingV14 = buildManualTimingImportScenesV14({ timingScenes, productionScenes, modeId: projectMode.id })
  const readiness = buildReadiness({ project, manualTiming, board, summary, modeId: projectMode.id, format, audio: assets.audio, timingScenes, productionScenes, assets })
  const codexTask = {
  ...
  const exportedStatus = readiness.full_package ? readiness.stage : (readiness.stage === 'prompts_ready' ? 'partial_package' : readiness.stage)
  ...
  const pack = {
    ...
    scenes: rootScenesForManualTimingV14,
    manual_timing_import_ready: {
      applied: true,
      scene_count: rootScenesForManualTimingV14.length,
    ...
    production: {
      ...
      fields_to_fill: codexTask.production_fields_to_fill || [],
