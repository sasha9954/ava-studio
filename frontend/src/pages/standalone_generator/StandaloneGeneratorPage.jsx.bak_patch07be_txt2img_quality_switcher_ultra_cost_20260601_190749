...
function readGeneratorGalleryDraft() {
  ...
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.map(normalizeGeneratorGalleryItem).filter(Boolean).slice(0, GENERATOR_GALLERY_LIMIT)
      : []
  } catch {
    return []
  }
...
export default function StandaloneGeneratorPage() {
  ...
  const historyIsFull = visibleHistoryItems.length >= historyLimit
    ...
    const tick = async () => {
      try {
        ...
        client_credit_cost: currentCreditCost,
            kind: (data.imageUrl || data.image_url) ? 'image' : 'video',
            title: (data.imageUrl || data.image_url) ? 'Фото' : 'Видео',
            toastTitle: (data.imageUrl || data.image_url) ? 'Фото готово' : 'Видео готово',
  ...
  }, [refreshCreditSummaryNow, updateCreditSummaryFromJobResponse, currentCreditCost])
...
<section className="avaGeneratorHistoryPanel">
          <div className="avaGeneratorHistoryHeader">
            ...
            <span>{visibleHistoryItems.length}/10</span>
          </div>

                    <div className="avaGeneratorHistoryCounts">
            <span>Фото: <strong>{historyImageCount}</strong></span>
            <span>Видео: <strong>{historyVideoCount}</strong></span>
            <span>Всего: <strong>{visibleHistoryItems.length}/{historyLimit}</strong></span>
          </div>
          {historyIsFull ? (
            <div className="avaGeneratorHistoryWarning">
              Лента переполнена. Удалите лишнее или нажмите «Очистить». Сейчас: фото {historyImageCount}, видео {historyVideoCount}, всего {visibleHistoryItems.length}/{historyLimit}.
            </div>
          ) : null}
