...
const AVA_STAGE101_META_PREFIX = "ava:podcast:persist:v1:";

function avaStage101PersistKey(sourceNodeId = "") {
  return `${AVA_STAGE101_META_PREFIX}${String(sourceNodeId || "workspace")}`;
}

function avaStage101SafeJsonParse(raw, fallback = null) {
  try { return raw ? JSON.parse(raw) : fallback; } catch { return fallback; }
}

function avaStage101OpenDb() {
  return new Promise((resolve, reject) => {
    const req = indexedDB.open(AVA_STAGE101_PODCAST_DB, 1);
    req.onupgradeneeded = () => {
      const db = req.result;
      if (!db.objectStoreNames.contains(AVA_STAGE101_ACTOR_STORE)) {
        db.createObjectStore(AVA_STAGE101_ACTOR_STORE);
      }
    };
...
async function avaStage101DbPut(key, value) {
  const db = await avaStage101OpenDb();
  try {
    await new Promise((resolve, reject) => {
      const tx = db.transaction(AVA_STAGE101_ACTOR_STORE, "readwrite");
      tx.objectStore(AVA_STAGE101_ACTOR_STORE).put(value, key);
...
async function avaStage101DbGet(key) {
  const db = await avaStage101OpenDb();
  try {
    return await new Promise((resolve, reject) => {
      const tx = db.transaction(AVA_STAGE101_ACTOR_STORE, "readonly");
      const req = tx.objectStore(AVA_STAGE101_ACTOR_STORE).get(key);
...
async function avaStage101DbDeletePrefix(prefix = "") {
  const db = await avaStage101OpenDb();
  try {
    await new Promise((resolve, reject) => {
      const tx = db.transaction(AVA_STAGE101_ACTOR_STORE, "readwrite");
      const store = tx.objectStore(AVA_STAGE101_ACTOR_STORE);
...
function avaStage101ActorBlobKey(sourceNodeId = "", actorId = "") {
  return `${String(sourceNodeId || "workspace")}::${String(actorId || "")}`;
}

function avaStage101StripRuntimeAudio(row = {}) {
  if (!row || typeof row !== "object") return row;
  const next = { ...row };
  ["url", "playbackUrl", "playback_url", "localUrl", "local_url", "objectUrl", "object_url"].forEach((key) => {
    if (String(next[key] || "").startsWith("blob:")) next[key] = "";
  });
  if (Array.isArray(next.blocks)) {
    next.blocks = next.blocks.map((block) => avaStage101StripRuntimeAudio(block));
  }
  return next;
}
...
function avaStage101WritePodcastPersist(sourceNodeId = "", payload = {}) {
  try {
    ...
    console.warn("[PAC STAGE101 PERSIST WRITE FAILED]", error);
  }
}

function avaStage101ReadPodcastPersist(sourceNodeId = "") {
  return avaStage101SafeJsonParse(localStorage.getItem(avaStage101PersistKey(sourceNodeId)), null);
}

async function avaStage101PersistActorBlobs(sourceNodeId = "", actorAudios = []) {
  const actors = Array.isArray(actorAudios) ? actorAudios : [];
  await Promise.all(actors.map(async (actor) => {
    const actorId = String(actor?.id || "").trim();
    if (!actorId) return;
    const blobKey = avaStage101ActorBlobKey(sourceNodeId, actorId);
    ...
    if (!runtimeUrl.startsWith("blob:")) return;

    try {
      const existing = await avaStage101DbGet(blobKey);
      if (existing) return;
      const blob = await fetch(runtimeUrl).then((res) => res.blob());
      await avaStage101DbPut(blobKey, {
        blob,
        name: actor?.filename || actor?.name || "actor_audio.mp3",
        mime: actor?.mime || blob.type || "audio/mpeg",
        durationSec: actor?.duration_sec || actor?.durationSec || actor?.audio_duration_sec || 0,
        savedAt: Date.now(),
      });
      console.log("[PAC STAGE101 ACTOR BLOB SAVED]", { actorId, blobKey, size: blob.size });
    } catch (error) {
      console.warn("[PAC STAGE101 ACTOR BLOB SAVE FAILED]", { actorId, error });
    }
  }));
}
...
async function avaStage101RestoreActorAudios(sourceNodeId = "", actorAudios = []) {
  ...
  for (const actor of actors) {
    ...
    const blobKey = actor.persistedBlobKey || avaStage101ActorBlobKey(sourceNodeId, actorId);
    const stored = await avaStage101DbGet(blobKey);
    if (!stored?.blob) {
      restored.push(actor);
      continue;
    }
    ...
    const restoredActor = {
      ...
      filename: actor.filename || stored.name || actor.name || "actor_audio.mp3",
      name: actor.name || actor.filename || stored.name || "actor_audio.mp3",
      mime: actor.mime || stored.mime || "audio/mpeg",
      duration_sec: actor.duration_sec || actor.durationSec || stored.durationSec || 0,
      durationSec: actor.durationSec || actor.duration_sec || stored.durationSec || 0,
      persistedBlobKey: blobKey,
    };

    if (Array.isArray(restoredActor.blocks)) {
      restoredActor.blocks = restoredActor.blocks.map((block) => ({
        ...block,
...
function avaStage101RepairBlocksWithActorUrls(blocks = [], actorAudios = [], savedClips = []) {
  ...
  return avaStage101SafeObjectRows(blocks, "blocks").map((block) => {
    ...
    const source = sources.find((row) => {
      const rowId = String(row?.id || row?.actor_id || row?.source_audio_id || row?.sourceAudioId || "").trim();
      return rowId === sourceId;
    });
    const sourceUrl = String(source?.url || source?.playbackUrl || source?.playback_url || source?.audio_url || source?.source_url || "").trim();
    if (!sourceUrl) return block;

    return {
      ...block,
      source_url: block.source_url || sourceUrl,
      audio_url: block.audio_url || sourceUrl,
      url: block.url || sourceUrl,
      playbackUrl: block.playbackUrl || sourceUrl,
      playback_url: block.playback_url || sourceUrl,
    };
  });
}

function avaStage101ClearPodcastPersist(sourceNodeId = "") {
  try { localStorage.removeItem(avaStage101PersistKey(sourceNodeId)); } catch {}
  void avaStage101DbDeletePrefix(`${String(sourceNodeId || "workspace")}::`).catch((error) => {
    console.warn("[PAC STAGE101 CLEAR DB FAILED]", error);
  });
}



function avaStage100PickAudioUrl(row = {}) {
...
function avaStage100FindAudioSourceForBlock(block = {}, actorAudios = [], savedClips = []) {
  const sourceId = String(block?.source_audio_id || block?.sourceAudioId || block?.source_id || block?.actor_id || "").trim();
  const ids = new Set([
    sourceId,
  ...
  const rows = [
    ...(Array.isArray(savedClips) ? savedClips : []),
    ...(Array.isArray(actorAudios) ? actorAudios : []),
  ];
  ...
  return rows.find((row) => {
    ...
    const rowLabels = [
      ...
      row?.block_label,
      row?.source_label,
      row?.inserted_phrase_label,
      row?.saved_clip_label,
      row?.phrase_label,
      row?.filename,
      row?.name,
      row?.source_name,
    ].map((value) => String(value || "").trim()).filter(Boolean);
    return rowLabels.some((label) => labels.has(label));
  }) || null;
}

function avaStage100ClearPodcastStorage(sourceNodeId = "") {
  const needle = String(sourceNodeId || "").trim().toLowerCase();
  const mustRemove = (key = "") => {
    const lower = String(key || "").toLowerCase();
    return (
      lower.includes("podcast") ||
      lower.includes("podcast_audio") ||
      lower.includes("podcast-audio") ||
      lower.includes("ava_manual_timing_podcast_return") ||
      lower.includes("ava:podcast-to-timing") ||
      lower.includes("manual_timing_podcast") ||
  ...
  try {
    Object.keys(localStorage).forEach((key) => {
      if (mustRemove(key)) localStorage.removeItem(key);
    });
  } catch {}

  try {
    Object.keys(sessionStorage).forEach((key) => {
      if (mustRemove(key)) sessionStorage.removeItem(key);
    });
  } catch {}
}

function avaStage100StopAudioElement(element) {
  if (!element) return;
...
function getAvaAuthHeaders(extra = {}) {
...
function repairDeadBlobAudioOnStartup(row = {}) {
  if (!row || typeof row !== "object") return row;
  const playback = String(row.playbackUrl || row.playback_url || row.localUrl || row.local_url || row.url || "").trim();
  if (!playback.startsWith("blob:")) return row;
...
function avaCoerceDownloadName(name = "file") {
  const raw = String(name || "file").trim() || "file";
  if (/^AVA_/i.test(raw)) return raw;
  ...
  return ext ? `AVA_${base}.${ext}` : `AVA_${base}`;
}

function getAudioUrlCandidate(row = {}) {
  if (!row || typeof row !== "object") return "";
  return String(
    row.playbackUrl ||
    row.playback_url ||
...
function copyAudioUrlFieldsFromSource(target = {}, source = {}) {
  if (!target || typeof target !== "object") return target;
  if (!source || typeof source !== "object") return target;
  const url = getAudioUrlCandidate(source);
  if (!url) return target;

  return {
    ...target,
    source_url: target.source_url || source.source_url || url,
    audio_url: target.audio_url || source.audio_url || url,
    fragment_url: target.fragment_url || source.fragment_url || url,
    src: target.src || source.src || url,
    url: target.url || source.url || url,
    asset_url: target.asset_url || source.asset_url || source.assetUrl || url,
    assetUrl: target.assetUrl || source.assetUrl || source.asset_url || url,
    server_url: target.server_url || source.server_url || source.asset_url || source.assetUrl || url,
    publicUrl: target.publicUrl || source.publicUrl || source.public_url || url,
    public_url: target.public_url || source.public_url || source.publicUrl || url,
  };
}

function blockLooksInsertedAudio(block = {}) {
  if (!block || typeof block !== "object") return false;
  const sourceId = String(block.source_audio_id || block.sourceAudioId || block.source_id || "").trim();
  return Boolean(
    block.type === "phrase" ||
    String(block.source_kind || "").includes("inserted") ||
    String(block.block_type || "").includes("inserted") ||
    sourceId && sourceId !== "main" && sourceId !== "silence" ||
    block.inserted_phrase_id ||
    block.saved_clip_id ||
    block.phrase_id ||
    block.inserted_phrase_label ||
...
function findAudioSourceForInsertedBlock(block = {}, actorAudios = [], savedClips = []) {
  if (!block || typeof block !== "object") return null;

  const ids = new Set([
    block.source_audio_id,
    block.sourceAudioId,
    block.source_id,
    block.actor_id,
    block.inserted_phrase_id,
  ...
  const rows = [...(Array.isArray(savedClips) ? savedClips : []), ...(Array.isArray(actorAudios) ? actorAudios : [])];
...
function repairInsertedAudioBlockUrls(blocks = [], actorAudios = [], savedClips = []) {
  let changed = false;
  const nextBlocks = (Array.isArray(blocks) ? blocks : []).map((block) => {
    if (!blockLooksInsertedAudio(block)) return block;
    if (getAudioUrlCandidate(block)) return block;

    const source = findAudioSourceForInsertedBlock(block, actorAudios, savedClips);
    if (!source || !getAudioUrlCandidate(source)) return block;

    changed = true;
    return copyAudioUrlFieldsFromSource(block, source);
  });
...
function avaSafeDownloadName(base = "file", ext = "") {
  const rawBase = String(base || "file").trim();
  const safeBase = rawBase
    ...
    .replace(/^_+|_+$/g, "")
    .slice(0, 72) || "file";
  const cleanExt = String(ext || "").replace(/^\./, "").trim().toLowerCase();
  return cleanExt ? `AVA_${safeBase}.${cleanExt}` : `AVA_${safeBase}`;
}
...
function isBackendStaticAssetUrl(url = "") {
  ...
  try {
    ...
    return parsed.pathname.startsWith("/static/assets/");
  } catch {
    return raw.startsWith("/static/assets/") || raw.includes("/static/assets/");
  }
}

function normalizeAvaStaticPlaybackUrl(url = "") {
  const raw = String(url || "").trim();
  if (!raw) return "";
  if (raw.startsWith("blob:") || raw.startsWith("data:")) return raw;
  try {
    const parsed = new URL(raw, typeof window !== "undefined" ? window.location.href : "http://localhost");
    if (parsed.pathname.startsWith("/static/assets/")) {
...
function isAvaProtectedAssetUrl(url = "") {
  ...
  try {
    const parsed = new URL(raw, typeof window !== "undefined" ? window.location.href : "http://localhost");
    return /^\/api\/assets\/[^/]+\/file/i.test(parsed.pathname) || /^\/assets\/[^/]+\/file/i.test(parsed.pathname);
  } catch {
    return /^\/api\/assets\/[^/]+\/file/i.test(raw) || /^\/assets\/[^/]+\/file/i.test(raw);
  }
}

function getAvaAudioPlaybackUrl(row = {}) {
  if (!row || typeof row !== "object") return "";
  return String(
    row.playbackUrl ||
    row.playback_url ||
...
function getFirstBackendStaticAssetUrl(row = {}, keys = SERVER_AUDIO_URL_KEYS) {
  ...
  for (const key of keys) {
    ...
    if (value && (isBackendStaticAssetUrl(value) || isAvaProtectedAssetUrl(value))) return value;
  }
  return "";
}
...
function getItemSourceUrl(item = {}, mainAudio = {}, actorAudios = [], savedClips = []) {
  if (!item || item.type === "silence" || item.source_audio_id === "silence") return "";

  const sourceId = String(item.source_audio_id || item.sourceAudioId || item.source_id || "main").trim() || "main";
  const directUrl = avaStage100PickAudioUrl(item);

  if (sourceId === "main" && !directUrl) {
    return avaStage100PickAudioUrl(mainAudio);
  }

  if (directUrl) return directUrl;

  const matched = avaStage100FindAudioSourceForBlock(item, actorAudios, savedClips);
  const matchedUrl = avaStage100PickAudioUrl(matched);
  if (matchedUrl) return matchedUrl;

  if (sourceId !== "main") {
    const actor = (Array.isArray(actorAudios) ? actorAudios : []).find((row) => {
      const rowId = String(row?.id || row?.actor_id || row?.source_audio_id || row?.sourceAudioId || "").trim();
      return rowId && rowId === sourceId;
    });
    const actorUrl = avaStage100PickAudioUrl(actor);
    if (actorUrl) return actorUrl;
  }

  return sourceId === "main" ? avaStage100PickAudioUrl(mainAudio) : "";
}
...
function createExternalAudioBlock({ sourceId, sourceUrl, sourceName, label, sourceLabel, color, startSec, endSec, colorIndex = 1 }) {
  return {
    id: createId("block"),
    type: "audio",
...
function isInsertedTimelineBlock(block = {}) {
  if (!block || typeof block !== "object") return false;
  const sourceId = String(block.source_audio_id || block.sourceAudioId || block.source_id || "main").trim();
  const sourceKind = String(block.source_kind || block.block_kind || block.block_type || "").trim().toLowerCase();

  if (block.type === "silence" || sourceId === "silence") return false;
  if (sourceId && sourceId !== "main") return true;
  if (sourceKind.includes("inserted") || sourceKind.includes("phrase") || sourceKind.includes("actor")) return true;
  return Boolean(
    block.inserted_phrase_id
    || block.saved_clip_id
    || block.phrase_id
...
function isEditableMainTimelineBlock(block = {}) {
  if (!block || typeof block !== "object") return false;
  const sourceId = String(block.source_audio_id || block.sourceAudioId || block.source_id || "main").trim() || "main";
  return sourceId === "main" && block.type !== "silence" && !isInsertedTimelineBlock(block);
}

function findEditableMainBlockToRight(blocks = [], fromIndex = -1) {
  const safeBlocks = Array.isArray(blocks) ? blocks : [];
  for (let index = fromIndex + 1; index < safeBlocks.length; index += 1) {
    if (isEditableMainTimelineBlock(safeBlocks[index])) return index;
  }
  return -1;
}

function createMainRemainderBlock(durationSec = 0, colorIndex = 0) {
  return {
    id: createId("block"),
    type: "audio",
    source_audio_id: "main",
    source_start_sec: 0,
    source_end_sec: roundSeconds(durationSec),
    color_index: colorIndex,
...
function isMainAudioBlockForInsert(block = {}) {
  if (!block || typeof block !== "object") return false;
  const sourceId = String(block.source_audio_id || block.sourceAudioId || block.source_id || "main").trim() || "main";
  const isPhrase = Boolean(
    block.type === "phrase"
    || block.block_kind === "phrase"
    || block.source_kind === "inserted_audio"
    || block.block_type === "inserted_audio"
    || block.inserted_phrase_id
    || block.saved_clip_id
    || block.phrase_id
    || block.inserted_phrase_label
  ...
  return sourceId === "main" && block.type !== "silence" && !isPhrase;
}

function getBlockSourceStart(block = {}, fallback = 0) {
  const value = Number(block?.source_start_sec);
  return Number.isFinite(value) ? roundSeconds(value) : roundSeconds(fallback);
}
...
function buildInsertedPhraseBlockFromSavedClip(clip = {}, clipAudioUrl = "", clipDuration = 0, fallbackColorIndex = 1) {
  ...
  const sourceStart = roundSeconds(clip.source_start_sec || 0);
  const computedDuration = Math.max(0, roundSeconds(clip.source_end_sec) - sourceStart);
  const duration = roundSeconds(clipDuration || clip.duration_sec || computedDuration);
  const sourceEnd = roundSeconds(clip.source_end_sec || (sourceStart + duration));
  const independentAssetUrl = String(clip.audio_url || clip.fragment_url || clip.asset_url || clip.assetUrl || clip.server_url || clip.publicUrl || clip.public_url || "").trim();
  const hasIndependentAsset = Boolean(independentAssetUrl && independentAssetUrl === clipAudioUrl && sourceStart <= 0.0005);
  return {
    id: createId("block"),
    type: "phrase",
    block_kind: "phrase",
    ...
    source_start_sec: hasIndependentAsset ? 0 : sourceStart,
    source_end_sec: hasIndependentAsset ? duration : sourceEnd,
    ...
    source_label: label,
    phrase_label: label,
    inserted_phrase_label: label,
    saved_clip_label: label,
    inserted_phrase_id: clip.id || createId("phrase"),
    saved_clip_id: clip.id || undefined,
  };
}

function insertBlockIntoTimelineAtTime(blocks = [], insertAtSec = 0, insertedBlock = {}) {
  const safeBlocks = Array.isArray(blocks) ? blocks : [];
  const duration = getBlockDuration(insertedBlock);
  if (!safeBlocks.length || duration <= 0) return null;

  const timelineDuration = getTimelineDuration(safeBlocks);
  const safeTime = clampSeconds(insertAtSec, 0, timelineDuration);
  const position = findTimelinePosition(safeBlocks, safeTime);

  const withSelected = (nextBlocks, insertedIndex) => ({
    blocks: nextBlocks,
    insertedIndex,
    insertedStartSec: getBlockVirtualStart(nextBlocks, insertedIndex),
  });

  if (!position) {
    const nextBlocks = [...safeBlocks, insertedBlock];
    return withSelected(nextBlocks, nextBlocks.length - 1);
  }

  const { index, block, offset_sec: rawOffset } = position;
  const blockDuration = getBlockDuration(block);
  const offset = clampSeconds(rawOffset, 0, blockDuration);

  if (offset <= 0.0005) {
    const nextBlocks = [...safeBlocks];
    nextBlocks.splice(index, 0, insertedBlock);
    return withSelected(nextBlocks, index);
  }

  if (offset >= blockDuration - 0.0005) {
    const nextBlocks = [...safeBlocks];
    nextBlocks.splice(index + 1, 0, insertedBlock);
    return withSelected(nextBlocks, index + 1);
  }

  if (isMainAudioBlockForInsert(block)) {
    // Critical podcast rule:
    // inserting actor/saved audio must break the original audio source into:
    // main left -> inserted phrase -> main right.
    // The right part continues from the exact original source time where the left part stopped.
    const sourceStart = getBlockSourceStart(block, getBlockVirtualStart(safeBlocks, index));
    const splitSourceTime = roundSeconds(sourceStart + offset);
    const sourceEnd = roundSeconds(block.source_end_sec);
    const leftBlock = {
      ...block,
      id: createId("block"),
    ...
    const rightBlock = {
      ...
      id: createId("block"),
      source_start_sec: splitSourceTime,
      source_end_sec: Math.max(splitSourceTime, sourceEnd),
    };
    const nextBlocks = [...safeBlocks];
    const replacement = [leftBlock, insertedBlock];
    if (getBlockDuration(rightBlock) > 0.0005) replacement.push(rightBlock);
    nextBlocks.splice(index, 1, ...replacement);
    return withSelected(nextBlocks, index + 1);
  }

  if (block.type === "silence") {
    const leftSilenceDuration = offset;
    const rightSilenceDuration = roundSeconds(blockDuration - offset);
    const replacement = [];
    if (leftSilenceDuration > 0.0005) {
      replacement.push({
        ...block,
        id: createId("block"),
        source_start_sec: 0,
        source_end_sec: leftSilenceDuration,
      });
    }
    const insertedIndex = index + replacement.length;
    replacement.push(insertedBlock);
    if (rightSilenceDuration > 0.0005) {
      replacement.push({
        ...block,
        id: createId("block"),
        source_start_sec: 0,
        source_end_sec: rightSilenceDuration,
      });
    }
    const nextBlocks = [...safeBlocks];
    nextBlocks.splice(index, 1, ...replacement);
    return withSelected(nextBlocks, insertedIndex);
  }

  // Do not split already inserted phrases. Put new insert after that locked block.
  const nextBlocks = [...safeBlocks];
  nextBlocks.splice(index + 1, 0, insertedBlock);
  return withSelected(nextBlocks, index + 1);
}

function replaceBlockWithInsertedPhrase(blocks = [], blockIndex = -1, insertedBlock = {}) {
  const safeBlocks = Array.isArray(blocks) ? blocks : [];
  const target = safeBlocks[blockIndex];
  if (!target) return null;

  // Replacing silence/old phrase is allowed. Replacing a main block would destroy original audio,
  // so for main blocks we insert at the current block start instead.
  if (target.type === "silence" || !isMainAudioBlockForInsert(target)) {
    const nextBlocks = [...safeBlocks];
    nextBlocks[blockIndex] = insertedBlock;
    return {
      blocks: nextBlocks,
      insertedIndex: blockIndex,
      insertedStartSec: getBlockVirtualStart(nextBlocks, blockIndex),
    };
  }

  return insertBlockIntoTimelineAtTime(safeBlocks, getBlockVirtualStart(safeBlocks, blockIndex), insertedBlock);
}
...
function buildContiguousBlocksFromDurations(blocks = [], durations = []) {
  ...
  let mainSourceCursor = null;
  let nonMainSinceLastMain = false;
  ...
  (Array.isArray(durations) ? durations : []).forEach((durationValue, index) => {
    ...
    const start = roundSeconds(timelineCursor);
    const end = roundSeconds(timelineCursor + duration);
    const storedLabel = getStoredBlockLabel(baseBlock);
    const isPhraseBlock = hasPhraseIdentity(baseBlock) || Boolean(storedLabel && baseBlock.type !== "silence" && !isMainAudioBlockForInsert(baseBlock));
    const type = baseBlock.type === "silence" ? "silence" : (isPhraseBlock ? "phrase" : (baseBlock.type || "audio"));
    const isMain = type !== "silence" && !isPhraseBlock && String(baseBlock.source_audio_id || "main").trim() === "main";

    let sourceStart = 0;
    let sourceEnd = duration;

    if (type === "silence") {
      sourceStart = 0;
      sourceEnd = duration;
      nonMainSinceLastMain = true;
    } else if (isPhraseBlock) {
      sourceStart = roundSeconds(baseBlock.source_start_sec);
      sourceEnd = roundSeconds(sourceStart + duration);
      nonMainSinceLastMain = true;
    } else if (isMain) {
      const baseStart = getBlockSourceStart(baseBlock, mainSourceCursor ?? start);
      if (mainSourceCursor === null) {
        sourceStart = baseStart;
      } else if (nonMainSinceLastMain) {
        // Inserted audio and silence do not consume original source audio.
        // After an insert, the next main block must continue from the previous main source end,
        // not from the visual timeline time.
        sourceStart = mainSourceCursor;
      } else {
        sourceStart = baseStart;
      }
      sourceEnd = roundSeconds(sourceStart + duration);
      mainSourceCursor = sourceEnd;
      nonMainSinceLastMain = false;
    } else {
      sourceStart = roundSeconds(baseBlock.source_start_sec);
      sourceEnd = roundSeconds(sourceStart + duration);
      nonMainSinceLastMain = true;
    ...
    nextBlocks.push({
      ...
      source_label: baseBlock.source_label || storedLabel || undefined,
...
function resizeSelectedBlockEnd(blocks = [], selectedIndex = -1, deltaSec = 0, audioDurationSec = 0) {
  ...
  const selectedBlock = safeBlocks[selectedIndex] || {};

  if (isInsertedTimelineBlock(selectedBlock)) {
    return {
      blocks: safeBlocks,
      selectedIndex,
  ...
  if (requestedDelta < 0) {
    // ←: shorten selected original/main block.
    // Freed duration goes only into the next original/main block.
    // Inserted actor/phrase blocks keep their own duration and only shift on the timeline.
    ...
    const targetIndex = findEditableMainBlockToRight(nextBlocksBase, selectedIndex);

    if (targetIndex >= 0) {
      nextDurations[targetIndex] = roundSeconds((nextDurations[targetIndex] || 0) + giveAmount);
    } else {
      // No original/main block on the right: create a main remainder after selected.
      nextDurations.splice(selectedIndex + 1, 0, giveAmount);
      nextBlocksBase.splice(selectedIndex + 1, 0, createMainRemainderBlock(
        giveAmount,
        ((Number(selectedBlock?.color_index) || 0) + 1) % BLOCK_COLORS.length
      ));
    }
    appliedDelta = roundSignedSeconds(-giveAmount);
  } else {
    // →: extend selected original/main block.
    // Take duration only from original/main blocks to the right.
    // Inserted actor/phrase blocks are locked and must not be shortened.
    let need = requestedDelta;
    let cursorIndex = selectedIndex + 1;

    while (need > 0.0005 && cursorIndex < nextDurations.length) {
      const candidate = nextBlocksBase[cursorIndex];
      if (!isEditableMainTimelineBlock(candidate)) {
        cursorIndex += 1;
        continue;
      }
  ...
  const nextRightEditableIndex = findEditableMainBlockToRight(nextBlocks, selectedIndex);
  const nextRightBlock = nextRightEditableIndex >= 0 ? nextBlocks[nextRightEditableIndex] : null;
...
export default function PodcastAudioComposerPage() {
  ...
  const [standaloneAudio, setStandaloneAudio] = useState(() => {
    try {
      ...
      return repairDeadBlobAudioOnStartup(normalizeManualTimingAudio(parsed));
    } catch {
  ...
  const handleStandaloneMainAudioUpload = async (event) => {
    ...
    try {
      ...
      const nextAudio = normalizeManualTimingAudio({
        ...
        assetId,
        asset_id: assetId,
        filename: uploaded.audio_name || uploaded.filename || uploaded.name || file.name || "podcast_audio",
        name: uploaded.audio_name || uploaded.filename || uploaded.name || file.name || "podcast_audio",
        duration_sec: Number(uploaded.audio_duration_sec || uploaded.duration_sec || uploaded.durationSec || 0),
        durationSec: Number(uploaded.audio_duration_sec || uploaded.duration_sec || uploaded.durationSec || 0),
        mime_type: uploaded.mime_type || uploaded.mime || file.type || "audio/mpeg",
        source: "podcast_main_playback",
      });
      setStandaloneAudio(nextAudio);
      setCurrentTimeSec(0);
      setIsPlaying(false);
      // PODCAST STANDALONE FORCE INIT: do not wait for old hydration logic.
      const uploadedDuration = roundSeconds(nextAudio.duration_sec || nextAudio.durationSec || 0);
      if (uploadedDuration > 0) {
        setDurationSec(uploadedDuration);
        const initialBlocks = createInitialBlocks(uploadedDuration);
      ...
      window.setTimeout(() => {
        void prepareAudioElement(nextAudio, 0);
      }, 50);
      try {
  ...
  useEffect(() => {
    ...
    void resolvePodcastPlaybackUrl(audio).then((playbackUrl) => {
      ...
      const mainSrc = new URL(playbackUrl, window.location.href).href;
      if (audioRef.current.src !== mainSrc && audioRef.current.currentSrc !== mainSrc) {
  ...
  const waitForAudioReady = (element) => new Promise((resolve) => {
    ...
    if (element.readyState >= 1 && Number.isFinite(element.duration)) return resolve(true);
    ...
    window.setTimeout(() => finish(element.readyState >= 1), 3000);
  });

  const resolvePodcastPlaybackUrl = async (source = "") => {
    const raw = typeof source === "object"
      ? String(getAvaAudioPlaybackUrl(source) || getAvaAudioServerUrl(source) || "")
      : String(source || "").trim();
    ...
    const isProtected =
      ...
      /\/assets\/[^/]+\/file/i.test(absoluteUrl);

    if (isBackendStaticAssetUrl(absoluteUrl) || isBackendStaticAssetUrl(raw)) {
      return normalizeAvaStaticPlaybackUrl(absoluteUrl);
    }
    if (!isProtected) return normalizeAvaStaticPlaybackUrl(raw);

    const cache = runtimeAudioBlobCacheRef.current || {};
    if (cache[absoluteUrl]) return cache[absoluteUrl];
    ...
    if (!response.ok) {
      console.warn("[PODCAST PLAYBACK FETCH FAILED]", { url: absoluteUrl, status: response.status });
      return "";
    }
    const blob = await response.blob();
    const blobUrl = URL.createObjectURL(blob);
    runtimeAudioBlobCacheRef.current = {
      ...(runtimeAudioBlobCacheRef.current || {}),
      [absoluteUrl]: blobUrl,
    };
    return blobUrl;
  ...
  // PODCAST REPAIR INSERTED URLS EFFECT
  useEffect(() => {
    setBlocks((items) => repairInsertedAudioBlockUrls(items, actorAudios, savedClips));
  }, [actorAudios, savedClips]);
  ...
  // PAC STAGE101 PERSIST RESTORE: actor audio + cuts survive F5.
  useEffect(() => {
    let cancelled = false;

    (async () => {
      const saved = avaStage101ReadPodcastPersist(sourceNodeId);
      if (!saved) {
        stage101HydratedRef.current = true;
        return;
      }
  ...
  useEffect(() => {
    if (!stage101HydratedRef.current) return;

    avaStage101WritePodcastPersist(sourceNodeId, {
      blocks,
      actorAudios,
      savedClips,
    });

    void avaStage101PersistActorBlobs(sourceNodeId, actorAudios);
  }, [sourceNodeId, blocks, actorAudios, savedClips]);
  ...
  const prepareBlockAudio = async (block, sourceTimeSec = null) => {
    ...
    const sourceUrl = getItemSourceUrl(block, audio, actorAudios, savedClips);
  ...
  const saveSelectedClip = () => {
    ...
    const clip = {
      ...
      source_url: getItemSourceUrl(block, audio, actorAudios, savedClips),
  ...
  const replaceSelectedWithSavedClip = (clip) => {
    ...
    const insertedBlock = buildInsertedPhraseBlockFromSavedClip(repairedClip, clipAudioUrl, clipDuration, snapshot.block.color_index);
    pushHistory("replace_with_saved_clip");

    const result = replaceBlockWithInsertedPhrase(blocks, snapshot.index, insertedBlock);
    if (!result) {
      setMessage("Не удалось вставить сохранённый кусок.");
      return;
    }

    setBlocks(result.blocks);
    setSelectedBlockId(result.blocks[result.insertedIndex]?.id || insertedBlock.id);
    seekOnTimeline(result.insertedStartSec, result.blocks);
    setBlockMenu(null);
    setMessage(
      snapshot.block.type === "silence" || !isMainAudioBlockForInsert(snapshot.block)
        ? `Выбранный блок заменён сохранённым куском “${clip.label}”.`
        : `Кусок “${clip.label}” вставлен в разрыв, основное аудио сохранено до и после вставки.`
    );
  };
  ...
  const insertSavedClipAfterSelected = (clip) => {
    ...
    const insertedBlock = buildInsertedPhraseBlockFromSavedClip(repairedClip, clipAudioUrl, clipDuration, snapshot.index + 1);
    pushHistory("insert_saved_clip_at_cut");

    // Insert at the yellow cut cursor when it is inside/near the selected block.
    // If the cursor is elsewhere, fallback to the selected block end.
    const selectedStart = snapshot.startSec;
    const selectedEnd = roundSeconds(snapshot.startSec + snapshot.durationSec);
    const cursorTime = clampSeconds(currentTimeSec, 0, getTimelineDuration(blocks));
    const cursorInsideSelected = cursorTime >= selectedStart - 0.001 && cursorTime <= selectedEnd + 0.001;
    const insertAtSec = cursorInsideSelected ? cursorTime : selectedEnd;

    const result = insertBlockIntoTimelineAtTime(blocks, insertAtSec, insertedBlock);
    if (!result) {
      setMessage("Не удалось вставить сохранённый кусок.");
      return;
    }
    ...
    seekOnTimeline(result.insertedStartSec, result.blocks);
    setBlockMenu(null);
    setMessage(`Кусок “${clip.label}” вставлен в разрыв. Основное аудио продолжится после вставки без наложения.`);
  };
  ...
  const playSavedClipPreview = async (clip) => {
    ...
    const resumeOffset = phrasePreview.clipId === clip.id && phrasePreview.positionSec < duration - 0.025
      ...
      : 0;
    const sourceUrl = getItemSourceUrl(clip, audio, actorAudios, savedClips);
  ...
  const seekSavedClipPreview = (clip, offsetSec) => {
    ...
    if (clip.type !== "silence" && audioRef.current) {
      prepareAudioElement(getItemSourceUrl(clip, audio, actorAudios, savedClips), roundSeconds(roundSeconds(clip.source_start_sec) + offset));
    }
  };
  ...
  const playFragmentAudioBlock = async (blockIndex, startOffsetSec = 0, mode = "selected_block") => {
    ...
    const savedClip = findSavedClipForBlock(block, savedClips);
    const fallbackSource = findAudioSourceForInsertedBlock(block, actorAudios, savedClips) || savedClip || {};
    const rawUrl = String(item.audioUrl || getItemSourceUrl(block, audio, actorAudios, savedClips) || getItemSourceUrl(fallbackSource, audio, actorAudios, savedClips) || "").trim();
    const playbackUrl = await resolvePodcastPlaybackUrl(rawUrl);
    const fragmentUrl = normalizeBrowserAudioUrl(playbackUrl || rawUrl);

    const duration = roundSeconds(item.durationSec || block.duration_sec || savedClip?.duration_sec || getBlockDuration(block));
    const sourceStartSec = roundSeconds(
      block?.source_start_sec ?? savedClip?.source_start_sec ?? item.sourceStartSec ?? 0
    );
    const sourceEndSec = roundSeconds(
      block?.source_end_sec ?? savedClip?.source_end_sec ?? (sourceStartSec + duration)
    );
    const startOffset = clampSeconds(startOffsetSec, 0, duration);
    const fragmentStartTime = roundSeconds(sourceStartSec + startOffset);

    if (!fragmentUrl) {
      console.warn("[PAC INSERTED_AUDIO_MISSING_URL_V174A]", { block, savedClip, fallbackSource });
      setMessage("У вставки нет живого audio_url/source_url — фраза пропущена. Сохрани/восстанови аудио актёра или пересоздай фразу.");
      if (mode === "sequence") {
        playNextSequenceBlock(blockIndex, blockIndex + 1);
      } else {
        activeMainPlaybackRef.current = null;
    ...
    const fragmentElement = new Audio(fragmentUrl);
    ...
    const tick = () => {
      ...
      const elementTime = Number.isFinite(fragmentElement.currentTime) ? fragmentElement.currentTime : fragmentStartTime;
      const sourceElapsed = Math.max(0, elementTime - sourceStartSec);
      const clockElapsed = startOffset + (performance.now() - startedAt) / 1000;
      const elapsed = Math.max(clockElapsed, sourceElapsed);
      setCurrentTimeSec(roundSeconds(blockStartSec + Math.min(duration, elapsed)));
      if (elapsed >= duration - 0.015 || elementTime >= sourceEndSec - 0.015) {
        finish();
        return;
      }
    ...
    fragmentElement.addEventListener("error", () => {
      console.warn("[PAC INSERTED_AUDIO_PLAYBACK_ERROR_V174A]", { fragmentUrl, rawUrl, block, savedClip });
      setMessage("Не удалось открыть вставленный аудио-фрагмент. Проверь, что аудио актёра не потеряно.");
    ...
    try {
      ...
      try {
        fragmentElement.currentTime = fragmentStartTime;
      } catch {}
      startedAt = performance.now();
      await fragmentElement.play();
      fragmentPlaybackRafRef.current = requestAnimationFrame(tick);
    } catch (error) {
      console.warn("[PAC INSERTED_AUDIO_OPEN_FAILED_V174A]", { error, fragmentUrl, rawUrl, block, savedClip });
      stopFragmentPlayback();
  ...
  async function playSequenceBlock(blockIndex, startOffsetSec = 0) {
    ...
    const ready = await prepareAudioElement(getItemSourceUrl(block, audio, actorAudios, savedClips) || audio, sourceTime);
    if (!ready) {
      stopMainPlayback();
  ...
  const togglePlayback = async () => {
    ...
    if (block) {
      ...
      const ready = await prepareAudioElement(getItemSourceUrl(block, audio, actorAudios, savedClips) || audio, roundSeconds(item.sourceStartSec + playOffset));
      if (!ready) {
  ...
  const onLoadedMetadata = (event) => {
    ...
    const mainPlaybackUrl = getAvaAudioPlaybackUrl(audio);
    const mainSrc = mainPlaybackUrl ? new URL(mainPlaybackUrl, window.location.href).href : "";
  ...
  const syncActorAudioMeta = (actorId, patch = {}) => {
    ...
    const nextLabel = hasLabelPatch ? String(patch.label || "").trim() : "";
    const nextColor = hasColorPatch ? String(patch.color || "").trim() : "";

    const actor = (Array.isArray(actorAudios) ? actorAudios : []).find((row) => String(row?.id || "") === safeActorId) || {};
    const knownLabels = new Set([
      actor.label,
      actor.name,
      actor.filename,
      ...(Array.isArray(actor.blocks) ? actor.blocks : []).flatMap((block) => [
        block.block_label,
        block.source_label,
        block.inserted_phrase_label,
        block.saved_clip_label,
    ...
    const rowBelongsToActor = (row = {}) => {
      if (!row || typeof row !== "object") return false;
      const rowSourceId = String(row.source_audio_id || row.sourceAudioId || row.source_id || row.actor_id || "").trim();
      if (rowSourceId === safeActorId) return true;
      const labels = [
        row.block_label,
        row.source_label,
        row.inserted_phrase_label,
      ...
      return labels.some((label) => knownLabels.has(label));
    };

    const patchLabelFields = (row = {}) => {
      if (!row || typeof row !== "object") return row;
      const next = { ...row };

      if (hasLabelPatch && nextLabel) {
        next.block_label = nextLabel;
        next.source_label = nextLabel;
        if (next.inserted_phrase_id || next.inserted_phrase_label) next.inserted_phrase_label = nextLabel;
        if (next.saved_clip_id || next.saved_clip_label) next.saved_clip_label = nextLabel;
        if (next.phrase_id || next.phrase_label) next.phrase_label = nextLabel;
        if (Object.prototype.hasOwnProperty.call(next, "label")) next.label = nextLabel;
      }
      if (hasColorPatch && nextColor) {
        next.color = nextColor;
      }
      return next;
    };

    setActorAudios((items) => (Array.isArray(items) ? items : []).map((item) => {
      if (String(item?.id || "") !== safeActorId) return item;
      return {
        ...item,
        ...(hasLabelPatch ? { label: nextLabel } : {}),
        ...(hasColorPatch ? { color: nextColor } : {}),
        blocks: (Array.isArray(item.blocks) ? item.blocks : []).map((block) => patchLabelFields({
          ...block,
          source_audio_id: block.source_audio_id || safeActorId,
        })),
      };
    }));

    setBlocks((items) => (Array.isArray(items) ? items : []).map((block) => (
      rowBelongsToActor(block) ? patchLabelFields(block) : block
    )));

    setSavedClips((items) => (Array.isArray(items) ? items : []).map((clip) => (
      rowBelongsToActor(clip) ? patchLabelFields(clip) : clip
    )));
  };


  const renameActorAudioLabel = (actorId, nextLabel) => {
    const safeActorId = String(actorId || "").trim();
    const safeLabel = String(nextLabel || "").trim();
    if (!safeActorId) return;

    setActorAudios((items) => (Array.isArray(items) ? items : []).map((actor) => {
      if (String(actor?.id || "") !== safeActorId) return actor;
      return {
        ...actor,
        label: safeLabel,
        blocks: (Array.isArray(actor.blocks) ? actor.blocks : []).map((block) => ({
          ...block,
          block_label: safeLabel,
    ...
    setBlocks((items) => (Array.isArray(items) ? items : []).map((block) => {
      if (String(block?.source_audio_id || block?.sourceAudioId || "") !== safeActorId) return block;
      return {
        ...block,
        block_label: safeLabel,
        source_label: safeLabel,
    ...
    setSavedClips((items) => (Array.isArray(items) ? items : []).map((clip) => {
      if (String(clip?.source_audio_id || clip?.sourceAudioId || "") !== safeActorId) return clip;
      return {
        ...clip,
        source_label: safeLabel,
        block_label: clip.block_label || safeLabel,
      };
    }));
  };

  const recolorActorAudio = (actorId, nextColor) => {
    const safeActorId = String(actorId || "").trim();
    const safeColor = String(nextColor || "").trim() || "#d99a18";
    if (!safeActorId) return;
    ...
    setActorAudios((items) => (Array.isArray(items) ? items : []).map((actor) => {
      ...
      return {
        ...
        blocks: (Array.isArray(actor.blocks) ? actor.blocks : []).map((block) => ({
          ...
          color: safeColor,
          block_label: block.block_label || actor.label,
          source_label: block.source_label || actor.label,
        })),
      };
    }));
  ...
  const adjustActorAudioBlockEnd = (...args) => {
    const actorId = String(args[0] || "").trim();
    if (!actorId) return;

    const rawDirection = args.find((value, index) => index > 0 && (
      typeof value === "number"
      || value === "left"
      || value === "right"
      || value === "prev"
    ...
    setActorAudios((items) => (Array.isArray(items) ? items : []).map((actor) => {
      if (String(actor?.id || "") !== actorId) return actor;

      const blocks = Array.isArray(actor.blocks) ? actor.blocks : [];
      if (!blocks.length) return actor;

      const selectedId = String(
        actor.selectedBlockId
        || actor.selected_block_id
        || actor.activeBlockId
        || actor.active_block_id
        || ""
      ).trim();

      let index = selectedId ? blocks.findIndex((block) => String(block?.id || "") === selectedId) : -1;
      if (index < 0) {
        index = blocks.findIndex((block) => block?.selected || block?.isSelected || block?.active);
      }
      if (index < 0) index = 0;

      const block = blocks[index] || {};
      const sourceStart = roundSeconds(block.source_start_sec ?? block.sourceStartSec ?? 0);
      const currentDuration = roundSeconds(
        block.duration_sec
        || block.durationSec
        || Math.max(0, roundSeconds(block.source_end_sec ?? block.sourceEndSec ?? 0) - sourceStart)
      );

      const actorDuration = roundSeconds(
        actor.duration_sec
        || actor.durationSec
        || actor.audio_duration_sec
        || actor.audioDurationSec
        || Math.max(currentDuration, roundSeconds(block.source_end_sec ?? block.sourceEndSec ?? currentDuration))
      );

      const step = clampSeconds(
        Number(actor.microStepSec || actor.micro_step_sec || actor.stepSec || actor.step_sec || actor.step || 0.1),
        0.001,
        30
      );

      const maxDuration = Math.max(0.01, roundSeconds(actorDuration - sourceStart));
      const nextDuration = clampSeconds(roundSeconds(currentDuration + direction * step), 0.01, maxDuration);
      const nextEnd = roundSeconds(sourceStart + nextDuration);

      if (Math.abs(nextDuration - currentDuration) < 0.0005) {
        return actor;
      }
      ...
      const nextBlocks = blocks.map((row, rowIndex) => {
        ...
        return {
          ...row,
          source_start_sec: sourceStart,
          source_end_sec: nextEnd,
          sourceStartSec: sourceStart,
          sourceEndSec: nextEnd,
          duration_sec: nextDuration,
          durationSec: nextDuration,
        };
      });

      console.log("[PAC ACTOR MICROSTEP]", {
        actorId,
        functionName: "adjustActorAudioBlockEnd",
        direction,
        step,
        blockId: block.id,
        sourceStart,
      ...
      return {
        ...actor,
        blocks: nextBlocks,
        selectedBlockId: blocks[index]?.id || selectedId,
      };
    }));
  };
  ...
  const adjustSelectedRightEdge = (direction) => {
    ...
    if (isInsertedTimelineBlock(blocks[selectedIndex])) {
      setMessage("Вставленный актёрский/сохранённый кусок заблокирован. Доводчик главной дорожки меняет только оригинальные части аудио.");
      return;
    }
  ...
  const clearAll = () => {
    // PODCAST_STAGE101_CLEAR_PERSIST
    avaStage101ClearPodcastPersist(sourceNodeId);

    // PODCAST_STAGE100_HARD_CLEAR
    avaStage100ClearPodcastStorage(sourceNodeId);
    avaStage100StopAudioElement(audioRef.current);
    try {
      Object.values(runtimeAudioBlobCacheRef.current || {}).forEach((url) => {
        if (String(url || "").startsWith("blob:")) URL.revokeObjectURL(url);
      });
      runtimeAudioBlobCacheRef.current = {};
    } catch {}

    // PODCAST STANDALONE HARD CLEAR
    stopSilencePlayback();
    stopPhrasePreview({ pause: false });
    stopActorPlayback({ pause: true });
    stopFragmentPlayback({ pause: true });
    stopMainPlayback();
    ...
    actorAudios.forEach((actor) => {
      try { if (String(actor?.url || "").startsWith("blob:")) URL.revokeObjectURL(actor.url); } catch {}
      void deleteActorAudioBlob(getActorAudioBlobKey(sourceNodeId, actor.id));
    });

    try {
      localStorage.removeItem(`ava_podcast_standalone_audio:${sourceNodeId}`);
      sessionStorage.removeItem(`ava_podcast_standalone_audio:${sourceNodeId}`);
      localStorage.removeItem(`ava_podcast_timing_return:${sourceNodeId}`);
      sessionStorage.removeItem(`ava_podcast_timing_return:${sourceNodeId}`);
    ...
    setStandaloneAudio(normalizeManualTimingAudio(null));
    setDurationSec(0);
    setCurrentTimeSec(0);
    setIsPlaying(false);
    setBlocks([]);
    ...
    hydratedRef.current = false;
    currentBlockIndexRef.current = 0;
    setMessage("Подкаст очищен. Загрузи основную аудио-дорожку заново.");
  };
  ...
  const renderComposerAudioBlob = async () => {
    ...
    try {
      ...
      for (const block of safeBlocks) {
        ...
        const sourceUrl = getItemSourceUrl(block, audio, actorAudios, savedClips);
        if (!sourceUrl) {
          if (isInsertedTimelineBlock(block) || hasPhraseIdentity(block)) {
            console.warn("[PAC RENDER_INSERTED_AUDIO_MISSING_URL_V174A]", block);
            throw new Error(`У вставленной фразы нет audio_url/source_url: ${getBlockLabelText(block) || block?.id || "phrase"}`);
          }
          outputCursor += blockFrames;
          continue;
        }
  ...
  const uploadFinalAudioBlob = async ({ blob, filename, mimeType = "" }) => {
    ...
    return {
      ...
      server_url: assetUrl,
      publicUrl: assetUrl,
      filename: data?.audio_name || data?.filename || data?.name || filename,
      name: data?.audio_name || data?.filename || data?.name || filename,
      duration_sec: roundSeconds(data?.audio_duration_sec || data?.duration_sec || data?.durationSec || 0),
      durationSec: roundSeconds(data?.audio_duration_sec || data?.duration_sec || data?.durationSec || 0),
    };
  };
  ...
  const uploadActorSourceAudioForServer = async (actor = {}) => {
    ...
    const playbackUrl = getAvaAudioPlaybackUrl(actor) || String(actor?.url || "");
    return {
      ...actor,
      url: playbackUrl || assetUrl,
  ...
  const prepareServerRenderableAudioSources = async () => {
    ...
    const originalUrl = String(getAvaAudioServerUrl(audio) || getAvaAudioServerUrl(location.state?.audio) || getAvaAudioServerUrl(storedManualTimingProject?.audio) || "").trim();
  ...
  const downloadComposedAudio = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = avaCoerceDownloadName(result.filename);
...
const applyComposedAudioToTiming = async () => {
  ...
  const selectedIndex = getSelectedBlockIndex(blocks, selectedBlockId);
  const selectedBoundaryAvailable = selectedIndex >= 0 && !isInsertedTimelineBlock(blocks[selectedIndex]);
  const canMoveSelectedRightBoundary = selectedIndex >= 0 && !isInsertedTimelineBlock(blocks[selectedIndex]);
                  ...
                  return (
                    <article className="podcastActorAudioCard" key={actor.id} style={{ "--actor-color": actor.color || "#d99a18" }}>
                      <div className="podcastActorAudioMeta">
                        ...
                        <label>
                          метка
                          <input value={actor.label} onChange={(event) => syncActorAudioMeta(actor.id, { label: event.target.value })} />
                        </label>
