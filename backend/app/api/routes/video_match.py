...
async def get_video_match_output(filename: str):
...
async def assemble_video_match_preview(payload: AssembleVideoMatchRequest = Body(...), _user=Depends(get_current_user)):
    ...
    try:
        ...
        if has_audio:
            ...
            if merged_has_audio:
                _run_ffmpeg([
                    "ffmpeg", "-y", "-i", str(merged_video), "-i", str(audio_input),
                    "-filter_complex", "[0:a]volume=1.0[a0];[1:a]volume=1.0[a1];[a0][a1]amix=inputs=2:duration=first:dropout_transition=0:normalize=0[aout]",
                    "-map", "0:v:0", "-map", "[aout]",
                ...
                _run_ffmpeg([
                    ...
                    "-c:v", "copy", "-c:a", "aac", "-ar", "48000", "-ac", "2", "-shortest", str(output_path),
                ])
        else:
